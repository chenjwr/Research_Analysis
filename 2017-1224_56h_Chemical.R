...
ZF_qPCR_Analysis_Function <- function(ChemicalList, TranscriptList,
                                      Direction, TimePt) {
  ...
  for (k in 1:length(ChemicalList)) {
    ...
    InputFile = 
      paste("~/Desktop/CSV_Files/2017-1223_XN_56h_",
    ...
    for (x in 1:length(TranscriptList)) {
      ...
      Ct_Ref_Target <- Ct_Ref_Target[colSums(!is.na(Ct_Ref_Target)) > nrow(TargetDF)]
      ...
      FileName = paste("~/Desktop/Output/", Target, "_", Chemical, 
                       "_", TimePt, ".csv", 
                       sep = "")
      ...
      FileName = paste("~/Desktop/Output/", Chemical, "_Summary.csv", 
                       sep = "")
...
ChemicalList = c("EHT1864-10uM",
                 "EHT1864-20uM",
                 "ML141-75uM",
                 "RO488071-50uM",
                 "Y27632-75uM")

TranscriptList = c("scxa",
                   "sox9a",
                   "col2a1",
                   "myod1")

ZF_qPCR_Analysis_Function(ChemicalList = ChemicalList, 
...
for (k in 1:length(ChemicalList)) {
  for (x in 1:length(TranscriptList)) {
    FileName = paste("~/Desktop/Output/", TranscriptList[x], "_",
  ...
  write.table(DF, file = "~/Desktop/Output/FoldChange_Chemical_Stats.csv",
              sep=",", row.names = FALSE, col.names = TRUE,
              append = FALSE)
...
for (k in 1:length(ChemicalList)) {
  ...
  FileName = paste("~/Desktop/Output/", 
                   ChemicalList[k], "_Summary.csv", sep = "")
  
  DataGraph_Summary <- read.csv(file = FileName,
                                header = FALSE, sep = ",", skipNul = TRUE, 
                                stringsAsFactors = FALSE)
  
  colnames(DataGraph_Summary) <- 
    c("Condition", "Transcript", "Summary", "TimePt",
      "Chemical", "Mean", "Std", "TechRep")
  ...
  DataGraph_Summary$Transcript = 
    factor(DataGraph_Summary$Transcript,
  ...
  qPCR_Plot <- 
    ...
    ggtitle("(32-56 hpf) _ ZF Summary") +
    xlab("") + 
  ...
  FileGraph = paste("~/Desktop/Output/FoldChange_", ChemicalList[k],
                    "_Stats.tiff", sep = "")
